...
import ai.gravityfield.gravity_sdk.models.PurchaseEvent
...
class PurchaseEventActivity : ComponentActivity() {
...
@Composable
fun PurchaseEventScreen() {
    val context = LocalContext.current
    var uniqueTransactionId by remember { mutableStateOf("") }
    ...
    Column(
        ...
        Text(
            text = "Purchase Event Configuration",
            modifier = Modifier.padding(bottom = 24.dp)
        )

        OutlinedTextField(
            value = uniqueTransactionId,
            onValueChange = { uniqueTransactionId = it },
            label = { Text("Unique Transaction ID") },
            modifier = Modifier
                .fillMaxWidth()
                    ...
                    cartItems.split(",").map { item ->
                        ...
                        if (parts.size >= 3) {
                            CartItem(
                                ...
                                itemPrice = parts[2].toDoubleOrNull() ?: 1.0
                            )
                        } else {
                            CartItem(
                                productId = parts.getOrElse(0) { "" },
                                quantity = parts.getOrElse(1) { "1" }.toIntOrNull() ?: 1,
                                itemPrice = parts.getOrElse(2) { "0.0" }.toDoubleOrNull() ?: 1.0
                            )
                        }
                    }
                } else {
